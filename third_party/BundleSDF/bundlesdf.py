...
class BundleSdf:
  def __init__(self, cfg_track_dir=f"{code_dir}/config_ho3d.yml", cfg_nerf_dir=f'{code_dir}/config.yml', start_nerf_keyframes=10, translation=None, sc_factor=None, use_gui=False, enable_nerf=True):
    ...
    self.enable_nerf = bool(enable_nerf)
    ...
    if self.enable_nerf:
      ...
      logging.info("Neural reconstruction disabled; BundleTrack pose frontend only")
  ...
  def find_corres(self, frame_pairs):
    ...
    for pair in query_pairs:
      raw_count = len(self.bundler._fm._raw_matches[pair])
      depth_count = len(self.bundler._fm._matches[pair])
      logging.info(
        f"correspondences {pair[0]._id_str}->{pair[1]._id_str}: "
        f"raw={raw_count}, valid_3d={depth_count}")
    ...
    for pair in query_pairs:
      logging.info(
        f"correspondences {pair[0]._id_str}->{pair[1]._id_str}: "
        f"ransac_inliers={len(self.bundler._fm._matches[pair])}")
  ...
  def run(self, color, depth, K, id_str, mask=None, occ_mask=None, pose_in_model=np.eye(4)):
    ...
    if self.enable_nerf and self.bundler._keyframes[-1]==frame:
